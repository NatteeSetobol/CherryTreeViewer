...
def Get_Main_Nodes():
    results = Query("select node.name,node.node_id from node, (SELECT COUNT(*) FROM children WHERE father_id = node.node_id) > 0 AS is_parent JOIN children ON node.node_id = children.node_id where children.father_id=(select node_id from children where master_id is NULL and father_id=0 LIMIT 1 ) ORDER BY node.name COLLATE NOCASE ASC", '')
